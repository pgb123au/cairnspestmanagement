# Cairns Pest Management — rank-and-rent microsite
...
| Item | Value |
|---|---|
| Domain | `cairnspestmanagement.com.au` (registered, GoDaddy fraud-pending) |
| Live URL (interim) | https://cairnspestmanagement.netlify.app |
...
│   ├── officer.astro         # /redlynch/
│   ├── berwick.astro         # /earlville/
│   ├── beaconsfield.astro    # /edmonton/
│   ├── cardinia.astro        # /cairns-region/ (shire-wide overview)
...
├── whisper.js                # "Pete's Lead from Cairns Pest Management" played to contractor before patching caller through
...
| `TENANT_BRAND` | `Cairns Pest Management` (default) | What plays after the label — full whisper is `"Pete's Lead. Pete's Lead from Cairns Pest Management."` |
...
3. Submit to Google Search Console (`sc-domain:cairnspestmanagement.com.au`).
4. Set up Google Business Profile (needs registered business name "Cairns Pest Management" first).
